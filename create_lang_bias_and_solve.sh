...
for ILASP_DIR in experiments/*/ ; do
    for MODEL in $ILASP_DIR*.damaged_model; do
        ...
        timeout 3600 ILASP --version=2i background_knowledge.lp $ILASP_DIR$LOG_NAME.search_space.lp $ILASP_DIR$FILENAME $ILASP_DIR$EX_NAME.examples > $ILASP_DIR$LOG_NAME.ILP_edits.lp
        exit_status=$?
